...
└── package_tests.json       # Конфигурация для тестов
```

## 🧪 Генерация тестовых ИНН

Для тестирования вы можете использовать утилиту `generateInn.js` (находится в `tests/utils/`):
...
const testINNs = generateINN.generateForTests(5, 'both');
```
